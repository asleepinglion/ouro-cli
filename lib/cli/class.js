...
module.exports = SuperJS.Application.extend({
  ...
  setup: function() {
    ...
    this.schedule('args', { setup: { modulePath: this._modulePath(__dirname), shortcut: 'args', local: true } });

  },

  beforeBoot: function() {

    this.log.info("SuperJS Version: " + SuperJS.version + "\n");
  },

  afterBoot: function() {

    //find commands provided as part of superjs
  ...
  findCommands: function(searchPath) {
    ...
    nodeModules.map(function(moduleName) {
      ...
      if( moduleName.indexOf('superjs-cli-') === 0 ) {
      ...
      } else if( moduleName.indexOf('superjs-') === 0 && fs.existsSync(searchPath + '/' + moduleName + '/node_modules')) {

        //recursively search for other cli command modules inside superjs packages
  ...
  routeRequest: function() {
    ...
    if( !self.commands[self.request.command] ) {
      ...
      return this.reject(new SuperJS.Error('invalid_request', 'The requested command was not found.'));
    }
